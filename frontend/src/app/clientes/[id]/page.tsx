...
import { useCostumersById } from "@/hooks/costumers.queries";
import { use, useEffect } from "react";
import { useUpdateCostumer } from "@/hooks/costumers.mutations";
...
export default function EditarClientePage({
  ...
  const costumerData = useCostumersById(id);
  const updateCostumer = useUpdateCostumer();
  ...
  useEffect(() => {
    if(costumerData.data) {
      setValue('address', costumerData.data.address);
      setValue('name', costumerData.data.name);
      setValue('email', costumerData.data.email);
      setValue('phone', costumerData.data.phone);
      setValue('birth', formatBirthDate(costumerData.data.birth));
    }
  }, [costumerData])

  const onSubmit = async(data: FormData) => {
    const body: UpdateCostumer = {
      ...data,
      birth: data.birth + 'T00:00:00.000Z'
    }
    try {
      await updateCostumer.mutateAsync({ id, body });
      router.push("/clientes")
    } catch (error: unknown) {
      console.log("error updateCostumer", error)
    }
  };
  ...
  return (
    <div className="">
      <div className="flex items-center mb-6">
        ...
        <h1 className="ml-2 text-2xl font-semibold text-black">Editando: {costumerData?.data?.name ? costumerData?.data?.name : 'Cliente'}</h1>
      </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div className="col-span-2 flex justify-end mt-6 space-x-4">
            ...
            <button type="submit" className="px-4 py-2 bg-green-600 text-white rounded-lg flex items-center hover:bg-green-700">
              Editar
              <Image src="/icons/edit-pencil-icon.svg" alt="Cadastrar" width={16} height={16} className="ml-2" />
            </button>
          </div>
        </form>
      </div>
